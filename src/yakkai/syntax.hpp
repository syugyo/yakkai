...
namespace yakkai
{
    namespace parser
    {
        ...
        public:
            ...
            auto parse_s_expression( RangedIterator& rng_it, bool const is_enable_closer = false )
                -> node*
            {
                skip_space( rng_it );
    ...
    template<typename RangedIterator, typename GC>
    auto parse_program( RangedIterator& rng_it, std::shared_ptr<GC> const& gc )
        -> node*
    {
        std::cout << "start parsing" << std::endl;
        parser::s_exp_parser<RangedIterator, GC> p( gc );
